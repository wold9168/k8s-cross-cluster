...
import (
	...
	dnsserver "github.com/wold9168/k8s-cross-cluster/sidecar/coredns-config-manager/pkg/dns-server"
)
...
func main() {
	...
	// 启动DNS服务器
	dnsServer := dnsserver.NewServer(":5353")
	...
	// 获取DNS服务器IP
	dnsIP, err := dnsServer.GetIP()
	if err != nil {
		klog.Errorf("Failed to get DNS server IP: %v", err)
		panic(err.Error())
	}
	klog.Infof("DNS server started on %s", dnsIP)
	...
	for {
		...
		// 检查Corefile(kube-system/configmaps/coredns)有没有设置我们拉起来的DNS服务器为上游。如果没有，则将对应的配置项置入
