...
# 单集群安装
just install --authkey your-headscale-preauth-key --login-server https://your-headscale-server --headscale-api-key your-headscale-api-key --context cluster1 --cluster-name na
just install --authkey your-headscale-preauth-key --login-server https://your-headscale-server --headscale-api-key your-headscale-api-key --context cluster2 --cluster-name nb

# 或批量安装（需先 export TS_AUTHKEY TS_LOGIN_SERVER HEADSCALE_API_KEY）
just install-all "cluster1:na cluster2:nb"
...
# 清理现场，卸载服务（单集群）
just uninstall ctx=cluster1
just uninstall ctx=cluster2

# 或批量卸载
just uninstall-all "cluster1:na cluster2:nb"
...
cd tailscale-manifest/lite-mode
# 10.96.0.0/12 是 k8s 创建服务所默认使用的 CIDR，集群创建时通过 --service-cluster-ip-range 参数控制
just install --authkey your-headscale-preauth-key --login-server https://your-headscale-server --headscale-api-key your-headscale-api-key --extra-args '--advertise-route=10.96.0.0/12' --context cluster1 --cluster-name na
just install --authkey your-headscale-preauth-key --login-server https://your-headscale-server --headscale-api-key your-headscale-api-key --extra-args '--advertise-routes=10.112.0.0/12' --context cluster2 --cluster-name nb
